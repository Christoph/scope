...
from last24h.models import Alert, Suggest, Query,UserProfile,Send,Sources
from last24h.forms import AlertEditForm,NameForm, AlertForm, RegistrationForm,ContactForm,RegistrationForm2,SourceForm
...
from graphite.celery import app
...
def register(request,):
    ...
    return render(request,'graphite/register.html', {'name':current_name,'form': form, 'state':state, 'log_inf':log_inf, 'log_link':log_link})
            # Save the user                                                                                                                                                 
...
def profile(request):    
    ...
    context = {'name':current_name, 'user':user, 'recent_queries':recent_queries, 'alert_info':alert_info, 'log_inf':log_inf, 'log_link':log_link, 'state':state,'form2':form2,'state_profile':state_profile}
...
#     log_inf, log_link = check_login(request)
#     return render(request, 'graphite/progress.html',{'name':current_name,'progress':data, 'task_id': job_id,'log_inf':log_inf, 'log_link':log_link})#HttpResponse(json.dumps(data),mimetype='application/json')
...
def customsearch(request):     
    ...
    form = SourceForm()
    log_inf, log_link = check_login(request)
    context = {'user_id':user_id, 'existingfeed':existingfeed,'form':form,'form2':form2, 'log_inf':log_inf, 'log_link':log_link,'name':current_name}
...
def grews_alert(request):
    ...
    form2 = SourceForm()
    log_inf, log_link = check_login(request)
    return render(request, 'graphite/set_alert.html',{'existingfeed':existingfeed,'state':state, 'form': form,'form2':form2,'log_inf':log_inf,'name':current_name, 'log_link':log_link})

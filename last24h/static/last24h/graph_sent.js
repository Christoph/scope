...
var select = d3.select("body").append("div").text("Sort by: ").attr("id","orderdiv").append("select").attr("id","order");//.on("change", change);
	...
	select.append("option").text("Sentimental analysis").attr("value","sent");
...
d3.json(tgt, function(error, root) {
  ...
  var path = chart.selectAll("path")
	...
	// 	.attr("class","span2")
	// 	.attr("data-slider-min","-20")
	// 	.attr("data-slider-max","20")
	// 	.attr("data-slider-step","1")
	// 	.attr("data-slider-value","-14")
	// 	.attr("data-slider-orientation","vertical")
	// 	.attr("data-slider-selection","after")
  ...
  var ang = (d.x + d.dx/2-Math.PI/2)/ Math.PI*180; // add +1/8*Math.PI in case you open up the initial circle
	...
	function around(d,centerx,centery,radius,newradius,height,clock){
	  	...
	  	if (clock == 3) {var radians = 1.5*d.sent*360*height/(360*newradius)+ -0.75*height/newradius; }
	...
	d3.select('#order').on('change', function() {
			  ...
			  if (this.value == "time") {
				...
				time6.transition().duration(2000).style("opacity",1);
					// circir.style("stroke",function(d) {return (d.suggest <= graph.graph.comps) && (d.suggest != 0) ? '#2b2b2b' :'#fff';}).style("opacity",1)
				...
				time5.transition().duration(2000).style("opacity",0);
				time6.transition().duration(2000).style("opacity",0);
				circir.style("fill",function(d) {return d.sent >= 0.5? '#2ca02c' :'#d62728';});
...
d3.json(json, function(error, graph) {
	...
	var sent_check = d3.select("body")
		...
		.style("right","150px")
		.style("top","60px" )
		.html('<span class="glyphicon glyphicon-adjust" aria-hidden="true"></span>')
		.on("click",function() {
				if (sent_display == 0) {sent_display = 1; 
										d3.selectAll("circle").style("fill",function(d) { 
												if ((1 >= d.sent) && (d.sent > 0.66)) {return '#2ca02c';}
												else if ((0.66 >= d.sent) && (d.sent > 0.33))  {return '#e7ba52';}
												else if ((0.33 >= d.sent) && (d.sent > 0))  {return '#d62728';}
											});
										d3.select('#ex1Slider').style("opacity",1).style("pointer-events","auto")
									}
				else {sent_display = 0; 
					d3.selectAll("circle").style("fill",function(d) { return color(d.comp);}).style("opacity",1)
					d3.select('#ex1Slider').style("opacity",0).style("pointer-events","none")
				}
		});
	...
	function gotitclick() { 
		...
		d3.select("#sent_check").transition().duration(3000).style("opacity",0.6).style("pointer-events","auto");
	...
	var node = svg//.append("g")  
	  ...
	  .on("mouseover",function(d){if (detail != 0){
	  		if (d.time != null) {
	  			div.html('<a href=' + d.url + '>' + d.title + '</a><br/><br/>' + d.source  + '   ' + d.time.substr(5,2) + '/' + d.time.substr(8,2) + '/' + d.time.substr(0,4) + ' ' + d.time.substr(11,5) + '<br/><br/>' + d.summary)
	  			}
	  		else {
	...
	// }
		...
		node.append("circle")
			...
			// .style("stroke",function(d) {return d.sent >= 0.5? '#2ca02c' :'#d62728';})
	...
	function init_positions() {
		...
		if (custom == 0) {
			...
			d3.select("#sent_check").transition().duration(3000).style("opacity",0.3);
			...
			chart.transition().duration(1000).style("opacity",1);
			d3.select('#gotit').style("pointer-events","auto").transition().duration(1000).style("opacity",0);
			d3.select("#sent_check").transition().duration(3000).style("opacity",0.6);
			d3.select("#helper").transition().duration(3000).style("opacity",0.6);
			d3.select("#suggestbutton").transition().duration(3000).style("opacity",0.6).style("pointer-events","auto");
			d3.select("#sharer").transition().duration(3000).style("opacity",0.6).style("pointer-events","auto");

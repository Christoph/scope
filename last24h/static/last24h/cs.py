...
for m in sorted(big_list,reverse=True):
    if ((False not in [ug.node[k]['suggest'] == 0 for k in ug.neighbors(m[1])]) and (ug.node[m[1]]['suggest'] == 0))and (len(list_corpus[m[1]]) > 1):
        ...
        keyword = []
        t = sorted([[float(i.split('*"')[0]),i.split('*"')[1].strip('" ')] for i in lsi_model.print_topics(n)[tops[0][0]].split('+') if (i.split('*"')[1].strip('" ') not in keywords_in) and (len(i) == 1)],reverse=True)
        if (len(t) != 0)and(t[0][0]>=0):
            keyword.append(t[0][1])
            keywords_in.append(t[0][1])                          
        t = sorted([[float(i.split('*"')[0]),i.split('*"')[1].strip('" ')] for i in lsi_model.print_topics(n)[tops[1][0]].split('+') if (i.split('*"')[1].strip('" ') not in keywords_in) and (len(i) == 1)],reverse=True)
        if (len(t) != 0)and(t[0][0]>=0):
            keyword.append(t[0][1])
            keywords_in.append(t[0][1])
...
with open('static/last24h/cs/cs_'+ strin +'_nl.json', 'w+') as fp:
    json.dump(ug_nl,fp)

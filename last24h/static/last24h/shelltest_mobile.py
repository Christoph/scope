...
thresh = 0.3#0.1+float(pow(articlenumber-500,2))/(float(pow(500,2))*10)#0.15#0.1/pow(upper/210,2)  #the higher the thresh, the less critical  
ug = nx.Graph()
...
for a in graphx:
	...
	#   count_comp += 1
		...
		print "mobile_cluster_desc"
		if cnode['time'] != None:
			mobile_cluster_desc = '<strong>' + keywords + '</strong><br/><p># of articles: '+ str(len(comp))+ '<p>Clustering: ' + str(clustering) + '%</br><a href=' + cnode['url'] + '>' + cnode['title'] + '</a><br/><br/>' + cnode['source']  + '   ' + cnode['time'][5:7] + '/' + cnode['time'][8:10] + ' ' + cnode['time'][11:16] + '<br/><br/>'
		else: 
			mobile_cluster_desc = '<strong>' + keywords + '</strong><br/><p># of articles: '+ str(len(comp))+ '<p>Clustering: ' + str(clustering) + '%</br><a href=' + cnode['url'] + '>' + cnode['title'] + '</a><br/><br/>' + cnode['source'] 
		print "add_nodes1"
		tg2.add_node(count_comp, clustering=clustering,name=mobile_cluster_desc, timeinf = timeinf, time_disclaim = time_disclaim)  

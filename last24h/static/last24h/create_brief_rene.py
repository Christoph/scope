...
#if len(list) == 0:
if select == 0:
	list = sorted([ug.node[i] for i in ug.nodes() if (0 < ug.node[i]['suggest'] <= max) ])
else:
	list = sorted([ug.node[i] for i in ug.nodes() if (ug.node[i]['suggest'] in select) ])
	#print select
	# <!-- Main -->

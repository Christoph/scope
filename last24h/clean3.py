...
def give_suggestions(graph,maxcount,filt= None):
    ...
    sug = sorted([[d['suggest'],k] for k,d in graph.nodes_iter(data=True)])
    ...
    if len(comp) > 5:
        for i in sorted(list(nx.find_cliques(comp)),key = len, reverse=True):
            if len(i) >= 3:
                ...
                if (False not in [ug.node[k]['suggest'] == 0 for k in ug.neighbors(susvec)]) and (ug.node[susvec]['suggest'] == 0):
                    #find keywords for it:
                    tops = []
                    for i in range(0,2):
                        tops.append(sorted(list_corpus[susvec],key= lambda close:close[1],reverse=True)[i])
                    #prop_o = int(math.floor(3./((tops[1][1]/tops[0][1])+1)))#1->1, 2->1,3->0,0.5->2, 0.3->3
                    keyword = []
                    t = sorted([[float(i.split('*"')[0]),i.split('*"')[1].strip('" ')] for i in lsi_model.print_topics(n)[tops[0][0]].split('+') if i.split('*"')[1].strip('" ') not in keywords_in],reverse=True)
    # for t in lsi_model.print_topics(n)[sorted(keyword_helper,reverse=True)[0][0]].split('+'):
                    #if t not in keywords_in:
                    if (len(t) != 0)and(t[0][0]>=0.2):
                        keyword.append(t[0][1])
                        keywords_in.append(t[0][1])
                          
                    t = sorted([[float(i.split('*"')[0]),i.split('*"')[1].strip('" ')] for i in lsi_model.print_topics(n)[tops[1][0]].split('+') if i.split('*"')[1].strip('" ') not in keywords_in],reverse=True)
                    ...
                    ug.node[susvec]['keywords'] = keyword
        ...
        for i in range(0,2):
            tops.append(sorted(list_corpus[susvec],key= lambda close:close[1],reverse=True)[i])
        #prop_o = int(math.floor(3./((tops[1]/tops[0])+1)))#1->1, 2->1,3->0,0.5->2, 0.3->3
        keyword = []
        t = sorted([[float(i.split('*"')[0]),i.split('*"')[1].strip('" ')] for i in lsi_model.print_topics(n)[tops[0][0]].split('+') if i.split('*"')[1].strip('" ') not in keywords_in],reverse=True)
    # for t in lsi_model.print_topics(n)[sorted(keyword_helper,reverse=True)[0][0]].split('+'):
        if (len(t) != 0)and(t[0][0]>=0.2):
        ...
        ug.node[susvec]['keywords'] = keyword#(',<br>').join(keyword)                                       
        if count_suggest <= maxcount:
            q = Suggest(custom= 'last24h', title = ug.node[susvec]['title'], url = ug.node[susvec]['url'], distance = count_suggest, images = ug.node[susvec]['images'])
            q.save()
        count_suggest += 1
        ...
        if count_suggest <= maxcount:
            if len(list_corpus[comp.nodes()[0]]) != 0:
                t = sorted([[float(i.split('*"')[0]),i.split('*"')[1].strip('" ')] for i in lsi_model.print_topics(n)[sorted(list_corpus[comp.nodes()[0]],key= lambda close:close[1],reverse=True)[0][0]].split('+') if i.split('*"')[1].strip('" ') not in keywords_in],reverse=True)
                if (len(t) != 0)and(t[0][0]>=0.2):
                    ug.node[susvec]['keywords'] = t[0][1]
        
            q = Suggest(custom= 'last24h', title = ug.node[susvec]['title'], url = ug.node[susvec]['url'], distance = count_suggest, images = ug.node[susvec]['images'])
            q.save()
        count_suggest += 1

#make suggest object for all articles but only give a distance to the selected ones...        

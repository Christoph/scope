...
def send_newsletter(customer_key):
	...
	message=mail_template(customer, articles, query, template_no, html=False),
	from_email='robot@scope.ai',
	recipient_list=recipients,
	html_message=mail_template(customer, send_dict, query, template_no)
)


def mail_template(customer, send_dict, query, no, html=True):
	if no == 1:
		if html == True:
